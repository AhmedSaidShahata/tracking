...
import {RoofService} from "./roofs-service";
...
@Component({
  ...
  styleUrls: ['./roofs.component.scss']
})
export class RoofsComponent implements OnInit {
  ...
  constructor(private _commondata: CommonDataService, private apiService: RoofService,
    ...
    apiService.getNationalities().subscribe(response => {
      this.towers = response.floors;
      this.allTowers = response.floors;
    });
  ...
  printExcel() {
    ...
    this.pdfServiceService.printFloor(this.allTowers);
  }
  ...
  ngOnInit() {
    ...
    if(ApiLinks.LANG == "ar"){
      this.columns[3]=  {prop: "tower.NameAr", name: this.tr.transform("buildings") };
      this.columns[4]=  {prop: "tower.location.NameAr", name: this.tr.transform("location") };

    }else{
      this.columns[3]=  {prop: "tower.NameEn", name: this.tr.transform("buildings") };
      this.columns[4]=  {prop: "tower.location.NameEn", name: this.tr.transform("location") };
  ...
  filterBuildings($event) {
    ...
    if (input == null) {
      ...
      // start filtering.
      Observable.from(this.allTowers).pipe(filter((item: Floor) => {
  ...
  public formSubmit(form: NgForm) {
    ...
    if (this.addUpdateTower.id == 0) {
      ...
      this.apiService.addNationality({
        ...
        TowerId: Number(this.selectedSubcat)
      })
        .subscribe(response => {
          // add success.
          this.addUpdateTower = this.getDefaultTower();
          this.allTowers.push(response.floors);
          this.towers = [...this.allTowers];
      ...
      this.apiService.updateNationality({
        ...
        this.allTowers[this.addUpdateTower.rowIndex] = response.floors;
        this.towers = [...this.allTowers];
        this.dialogService.SuccesMessage();
        this.addUpdateTower = this.getDefaultTower();
      }, error1 => {
