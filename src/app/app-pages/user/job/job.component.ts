...
export class JobComponent implements OnInit {
  ...
  constructor(private _commondata: CommonDataService, private apiService: JobService,
    ...
    if (this.addUpdateLocation.id == 0) {
      ...
      this.apiService.addNationality(this.addUpdateLocation.NameAr, this.addUpdateLocation.NameEn)
        .subscribe(response => {
          ...
          this.allLocations.push(response.job);
          this.locations = [...this.allLocations];
      ...
      this.apiService.updateNationality(this.addUpdateLocation, this.addUpdateLocation.id).subscribe(response => {
        this.allLocations[this.addUpdateLocation.rowIndex] = response.job;
        this.locations = [...this.allLocations];
        this.addUpdateLocation = this.getDefaultLocation();
      }, error1 => {
        console.error(error1);
  ...
  private loadDepartment() {
    this.apiService.getJobs().subscribe(response => {
      this.locations = response.jobs;
      this.allLocations = response.jobs;
